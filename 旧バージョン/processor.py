...
def trim_whitespace(image_path: str, output_path: str, keep_margin_percent: int, color_type: str, direction: str):
    ...
    - 上書き保存時のファイルロック回避のため、with句で開き、処理後に保存する
    """
    cropped_final = None
    save_format = None
    
    # 1. 画像を開いてメモリ上で処理完了させる（ファイルハンドルを確実に解放するため）
    with Image.open(image_path) as orig:
        # Exif回転などを加味するかはPillow任せだが、基本はそのまま開く
        w, h = orig.size
        ...
        # 切り抜き（Copyして実体をメモリに持つ）
        cropped = orig.crop((l, t, r, b)).copy()
        ...
        if keep_margin_percent > 0:
            ...
            # 中央にペースト
            canvas.paste(cropped, (mw, mh), cropped if _has_alpha(cropped) else None)
            cropped_final = canvas
        else:
            cropped_final = cropped

        # 保存形式の決定ロジック用の情報を保持
        if _has_alpha(cropped_final) or cropped_final.mode not in ("RGB", "L"):
            # 透過あり or 特殊モード -> 保存時に形式依存
            pass 
        else:
            # 単純RGB
            pass

    # 2. ここで orig はClose済み。保存を行う。
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    ext = os.path.splitext(output_path)[1].lower()
    
    if ext in (".jpg", ".jpeg", ".bmp"):
        # JPEG/BMP等はアルファ不可なのでRGB化
        if _has_alpha(cropped_final) or cropped_final.mode not in ("RGB", "L"):
            cropped_final = cropped_final.convert("RGB")
        try:
            cropped_final.save(output_path, quality=95)
        except Exception:
            # 画質指定できない形式(BMP等)のフォールバック
